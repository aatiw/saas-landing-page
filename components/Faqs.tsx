  ...
  return (
    <section className="py-24">
      <div className="container">
        ...
        <h2 className="text-6xl font-medium mt-6 text-center mx-auto text-white">
          Questions? We&apos;ve <br className="hidden lg:block" /> got <span className="text-lime-400">answers</span>
        </h2>
      </div>
        ...
        {faqs.map((faq, faqIndex) => (
          <div key={faq.question} className="bg-neutral-900 rounded-2xl border border-white/10 p-6">
            ...
            <div className={twMerge("mt-6", selectedIndex !== faqIndex && "hidden")}>
              <p className="text-white/30">{faq.answer}</p>
            </div>
          </div>
        ))}
      </div>
